			 ...
			 unsigned long now)
{
	if ((watchdog_enabled & WATCHDOG_SOFTOCKUP_ENABLED) && watchdog_thresh) {
		...
		/*
		 * Long running BPF programs can cause CPU's to stall. So trigger
		 * fast path termination to terminate such BPF programs.
		 */
		if (time_after_eq(now, period_ts + get_softlockup_thresh() * 1 / 4))
